...
import { FileText, Plus, Eye, Wallet, MoreVertical, X, Download, Printer, Calendar, DollarSign, CreditCard, Hash, FileEdit } from 'lucide-react';
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const [invRes, orgRes] = await Promise.all([
        deliveryOrderService.getDeliveryOrders('billed', 1, 100),
              ...
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-800 font-semibold">{inv.customer?.name}</td>
                    <td className="px-6 py-4 text-right">{inv.total_ready_gazana} GZ</td>
